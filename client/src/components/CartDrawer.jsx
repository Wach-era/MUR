  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur-sm">
        <div className="w-full max-w-md bg-[#120a1f] border-l border-purple-500/20 text-white flex flex-col h-full shadow-2xl">
          
          {/* Header */}
          <div className="p-5 border-b border-purple-900/40 flex items-center justify-between">
            <h2 className="text-lg font-bold flex items-center gap-2">
              🛒 Your Cart <span className="text-xs text-purple-400 font-normal">({cartItems.length} items)</span>
            </h2>
            <button 
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            {cartItems.length === 0 ? (
              <div className="text-center py-16 text-purple-300/60">
                ...
                <p className="text-xs text-purple-300/40 mt-1">Explore the feed to find vinyls and merch!</p>
              </div>
            ) : (
              cartItems.map((item) => {
                ...
                return (
                  <div 
                    key={item._id} 
                    className={`glass-card p-3 rounded-xl flex gap-3 items-center bg-black/20 border ${
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-purple-300/70 truncate">{item.artist} ({item.format})</p>
                      
                      {/* Stock Warning Badge */}
                      {isOutOfStock ? (
                        <span className="inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded bg-red-500/20 text-red-400 border border-red-500/30">
                          OUT OF STOCK
                        </span>
                      ) : isExceedingStock ? (
                    ...
                    {/* Quantity Controls */}
                    <div className="flex items-center gap-2 bg-black/40 border border-purple-500/20 rounded-lg p-1">
                      <button 
                        onClick={() => updateQuantity(item._id, itemQty - 1)}
                        className="w-6 h-6 rounded bg-purple-900/40 text-purple-200 hover:bg-purple-800 text-xs flex items-center justify-center font-bold transition-colors"
                      >
                        -
                      </button>
                      <span className="text-xs font-semibold px-1 min-w-[20px] text-center">{itemQty}</span>
                      <button 
                        onClick={() => updateQuantity(item._id, itemQty + 1)}
                        disabled={itemQty >= availableStock}
                        className="w-6 h-6 rounded bg-purple-900/40 text-purple-200 hover:bg-purple-800 text-xs flex items-center justify-center font-bold transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                      >
                        +
                      </button>
                    </div>
          ...
          {/* Footer */}
          {cartItems.length > 0 && (
            <div className="p-5 border-t border-purple-900/40 bg-black/30 space-y-4">
              ...
              <p className="text-xs text-purple-300/50 text-center">Taxes &amp; shipping calculated at checkout</p>
              
              <button 
                onClick={handleProceedToCheckout}
                disabled={isCheckoutDisabled}
                className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-emerald-900/40 transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:from-emerald-600"
              >
                <span>
      ...
      {isMpesaModalOpen && (
        ...
        >
          <div 
            className="w-full max-w-md bg-[#120a1f] border border-emerald-500/30 rounded-2xl p-6 shadow-2xl relative text-white space-y-4"
          ...
          >
            <div className="flex justify-between items-center border-b border-purple-900/40 pb-3">
              <h3 className="text-lg font-bold flex items-center gap-2 text-emerald-400">
                <span>📲</span> M-Pesa Express Checkout
              </h3>
              {!isProcessing && (
                <button 
            ...
            <div className="bg-emerald-950/40 border border-emerald-500/20 p-3 rounded-xl flex justify-between items-center">
            ...
            <form onSubmit={handleInitiateMpesa} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-[#1c122e] border border-purple-500/30 rounded-xl px-4 py-3 text-white placeholder-purple-400/40 focus:outline-none focus:border-emerald-400 text-sm"
                />
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
              >
                {isProcessing ? 'Initiating Prompt...' : `Send M-Pesa STK Push (Ksh ${Math.round(Number(subtotal) || 0)})`}
              </button>
            </form>
          </div>
        </div>
